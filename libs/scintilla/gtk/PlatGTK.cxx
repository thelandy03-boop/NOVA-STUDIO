...
void SurfaceImpl::FillRectangle(PRectangle rc, ColourDesired back) {
	PenColour(back);
	if (context && (rc.left < maxCoordinate)) {
...
void ListBoxX::SetFont(Font &font) {
	if (Created() && PFont(font)->pfd) {
		...
		if (cssProvider) {
			...
			if (gtk_check_version(3, 21, 0) != NULL ||
...
#	if GTK_CHECK_VERSION(3,20,0)
		...
		gtk_widget_path_append_type(widget_path, GTK_TYPE_BORDER);
...
public:
	explicit DynamicLibraryImpl(const char *) {}
	~DynamicLibraryImpl() override {}
	Function FindFunction(const char *) override { return nullptr; }
	bool IsValid() override { return false; }
};

DynamicLibrary *DynamicLibrary::Load(const char *) {
	return nullptr;
}
